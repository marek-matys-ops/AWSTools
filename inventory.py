	...
	for index, service in enumerate(services):
		...
		for region in allRegions:
			if service[3] == True: # One pass for global services
				#dict_arg = service[6]
				x = threading.Thread(target=get_service_count, args=(service[0], service[1], "us-east-1", service[2], service[3], service[4], service[5], kwargs=service[6]) )				   
				threads.append(x)
				x.start()				
				break
			else:
				x = threading.Thread(target=get_service_count, args=(service[0], service[1], region, service[2], service[3], service[4], service[5], kwargs=service[6]))
				threads.append(x)
				x.start()
